...
public class ObjectCollectionManager : Singleton<ObjectCollectionManager>
{
    ...
    [Tooltip("A collection of Hotspot prefabs to generate in the world.")]
    public List<GameObject> HotSpotPrefabs;

    [Tooltip("The desired size of the hotspot.")]
    public Vector3 HotSpotSize = new Vector3(1.0f, .5f, .5f);
    ...
    public void CreateHotSpot(int number, Vector3 positionCenter, Quaternion rotation)
    {
        CreateBuilding(HotSpotPrefabs[number], positionCenter, rotation, HotSpotSize);
    }
    ...
    private void CalculateScaleFactor()
    {
        ...
        var ratio = CalcScaleFactorHelper(HotSpotPrefabs, HotSpotSize);
        if (ratio < maxScale)
        {
            maxScale = ratio;
        }
